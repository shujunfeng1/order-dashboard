...
程序从INBOX最新邮件开始向前读取头信息，再按发件人和解码后的中文主题匹配目标报表，避免企业邮箱在大收件箱上执行服务端IMAP SEARCH时超时。默认最多扫描最近500封邮件。每个更新时段会校验对应报表是否已经到达；若邮件延迟或出现临时网络异常，工作流会间隔60秒重试，连续失败则保留上一版线上数据。
