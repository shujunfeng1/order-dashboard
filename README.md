...
| 客户登录、拜访及转化看板 | https://shujunfeng1.github.io/order-dashboard/customer-login-dashboard.html | 每日16:45（北京时间） |
...
程序从INBOX最新邮件开始向前读取头信息，再按发件人和解码后的中文主题匹配目标报表，避免企业邮箱在大收件箱上执行服务端IMAP SEARCH时超时。默认最多扫描最近500封邮件。每天16:45更新时会校验当天16:40报表是否已经到达；若邮件延迟或出现临时网络异常，工作流会间隔60秒重试，连续失败则保留上一版线上数据。
...
.github/workflows/
  ...
  update-customer-login-dashboard.yml  登录看板每日16:45更新
config/
  settings.json                        卡单配置
...
## 2026-09-17 登录看板调整为每日一次更新

- 客户登录看板不再获取10:45和13:15报表，只使用每天16:40的最终报表。
- GitHub Actions调整为每天北京时间16:45运行，对应UTC cron为`45 8 * * *`。
- `report_slots`同步收敛为`16:40`，严格时段校验不会使用当天较早邮件或前一天邮件代替。
- 卡单看板的更新计划和客户登录看板的指标口径均未改变。

## 2026-07-27 登录看板IMAP超时修复

### 现象与根因

- 北京时间2026-07-27 16:49启动的登录看板任务连续3次失败，线上数据停留在13:15。
- 16:40报表实际已于16:41:36收到，不是邮件延迟或缺失。
- INBOX当时共有15794封邮件，原程序执行`SEARCH SINCE ... FROM ...`时每次均在30秒后读取超时。
- `finally`中的`logout()`又对已超时连接执行读操作，产生二次`OSError`，掩盖了最初的SEARCH超时。

### 修复方案与验证

- 取消服务端全收件箱SEARCH，改为按邮件序号从最新邮件向前扫描头信息，再在本地匹配发件人、主题和报表时间。
- 新增`scan_recent_messages`配置，默认最多检查最近500封邮件；正常场景下目标报表通常位于最新几封。
- 邮箱退出时同时忽略IMAP协议异常和已超时socket的`OSError`，避免二次异常覆盖真实根因。
- 工作流重试提示改为同时覆盖“邮件延迟”和“临时网络异常”，避免错误归因为邮件未到。
- 使用真实邮箱验证成功读取`2026-07-27 16:40`报表：59979行、59963个去重客户，完整管道耗时约47秒。
- 单元测试新增“禁止调用服务端SEARCH”和“拒绝旧时段报表”两项回归校验。
